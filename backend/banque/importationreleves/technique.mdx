...
<Card title="ImportTemplateController (Package : ...controller)">
  **Rôle :** Expose les API REST pour la gestion des modèles d'importation.
  
  **Endpoints :**
  <Callout type="info" title="Endpoints CRUD pour /api/v1/import-templates">
...
<Card title="StatementImportController (Package : ...controller)">
  **Rôle :** Expose l'API REST pour le téléversement des fichiers de relevés.
  
  **Endpoint :**
  <Callout type="info" title="POST /api/v1/accounts/{accountId}/statements/upload">
    *   Téléverse un fichier de relevé.
...
<Card title="ImportTemplateService (Package : ...service)">
  ...
  **Méthodes Clés :**
  <Callout type="info" title="Opérations de Lecture">
    `getAllTemplates()`, `getTemplateById(UUID)`, `getTemplatesForBank(UUID)`
  </Callout>
  <Callout type="info" title="createTemplate(ImportTemplateDto)">
  ...
  <Callout type="info" title="updateTemplate(UUID templateId, ImportTemplateDto)">
  ...
  <Callout type="info" title="deleteTemplate(UUID templateId)">
    Supprime le modèle. La suppression en cascade (via `CascadeType.ALL` et `orphanRemoval=true`) devrait supprimer les `TemplateFieldMappingEntity` associés.
  </Callout>
</Card>

<Card title="StatementProcessingService (Package : ...service)">
  ...
  **Méthodes Clés :**
  <Callout type="info" title="parseStatement(MultipartFile file, ImportTemplateEntity template)">
    Méthode principale.
...
<Card title="ImportTemplateEntity (Package : ...entity)">
  ...
  **Champs Principaux :**
  ...
  | `name`            | String, unique                                                                                       | Nom du modèle.                                                                                          |
  ...
  | Audit Champs      | `createdByUser`, `updatedByUser`, `createdAt`, `updatedAt`                                             | Champs d'audit standards.                                                                               |
  | `fieldMappings`   | OneToMany vers `TemplateFieldMappingEntity` (`CascadeType.ALL, orphanRemoval = true, FetchType.LAZY`) | Liste des mappings de champs. `orphanRemoval=true` est crucial pour la gestion du cycle de vie.       |
</Card>

<Card title="TemplateFieldMappingEntity (Package : ...entity)">
...
<Callout type="info" title="Principaux DTOs">
...
<Card title="ImportTemplateMapper (utilise TemplateFieldMappingMapper)">
...
<Card title="TemplateFieldMappingMapper">
...
<Card title="ImportTemplateRepository : JpaRepository<ImportTemplateEntity, UUID>">
...
<Card title="TemplateFieldMappingRepository : JpaRepository<TemplateFieldMappingEntity, UUID>">
...
<Card title="SourceType (Enum)">
`EXCEL, CSV, API, MANUAL`. Définit les types de fichiers sources.
</Card>

## 3. Workflow d'Importation (Focus Technique)
<Card title="Workflow : Importation de Relevé">
